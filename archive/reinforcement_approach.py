...
with tf.Session() as sess:
    ...
    for i in range(EPISODES_NUM):
        ...
        actions_buffer = np.zeros(shape=(GAME_SIZE, GAME_SIZE), dtype=np.float32)
        ...
        for j in range(GAME_SIZE):
            ...
            if reward == 1:
                ...
                one_hot_reward = np.eye(GAME_SIZE)[chosen_action]
                actions_buffer[j + 1] = one_hot_reward
                arg_max_buffer[j + 1] = chosen_action

                # Calculate the discounted reward. Q(t) = Q(t) + Y * Max:Q(t + 1).
                for k in reversed(range(j)):
                    one_hot_reward = np.eye(GAME_SIZE)[arg_max_buffer[k]] * reward
                    actions_buffer[k] += one_hot_reward
                    reward *= DISCOUNTED_RATE
        ...
        if i % 1000 == 0:
            print(np.argmax(actions_buffer[0]), np.argmax(actions_buffer[1]), np.argmax(actions_buffer[2]))
            print('At Ep:', i, '\tWins Were:', wins, '\t Loses Were: ', loses)
            wins, loses = 0, 0
